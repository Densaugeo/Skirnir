...
skirnir.on('add', function(e) {
  console.log('Added'.green + ' new serial device: ' + e.device.green + '. Current devices are:');
  
  list_keys(skirnir.devices);
});

skirnir.on('remove', function(e) {
  console.log('Removed'.red + ' serial device: ' + e.device.green + '. Current devices are:');
  
  list_keys(skirnir.devices);
});

skirnir.on('connect', function(e) {
  console.log('Connected'.green + ' device ' + e.device.green + '. Current connections are:');
  
  list_keys(skirnir.connections);
});

skirnir.on('disconnect', function(e) {
  console.log('Disconnected'.red + ' device ' + e.device.green + '. Current connections are:');
